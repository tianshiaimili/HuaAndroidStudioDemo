...
public class RoundImageView extends ImageView {
        ...
        if (type == TYPE_CIRCLE) {
            ...
            Log.e("TAG",
                    "b'w = " + bmp.getWidth() + " , " + "b'h = "
                            + bmp.getHeight());
            scale_width = bmp.getWidth()*1.0f/getWidth() > 1 ? getWidth() * 1.0f /bmp.getWidth():bmp.getWidth()*1.0f/getWidth();
            scale_height = bmp.getHeight()*1.0f/getHeight() > 1 ? getHeight()*1.0f/bmp.getHeight(): bmp.getHeight()*1.0f/getHeight();

                // 如果图片的宽或者高与view的宽高不匹配，计算出需要缩放的比例；缩放后的图片的宽高，一定要大于我们view的宽高；所以我们这里取大值；
                scale = Math.max(getWidth() * 1.0f / bmp.getWidth(),
                        getHeight() * 1.0f / bmp.getHeight());
            scale = calculateInSampleSize(bmp.getWidth(), bmp.getHeight());

        }
        // shader的变换矩阵，我们这里主要用于放大或者缩小
        mMatrix.setScale(scale_width, scale_height);
        // 设置变换矩阵
    ...
    public int calculateInSampleSize(int width, int height) {
        ...
        if (height > getHeight() || width > getWidth()) {
            if (width > height) {
                inSampleSize = Math.round((float)height / (float)getHeight());
            } else {
                inSampleSize = Math.round((float)width / (float)getWidth());
            }
        }
        return inSampleSize;
    }
